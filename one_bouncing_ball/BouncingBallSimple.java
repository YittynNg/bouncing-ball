...
public class BouncingBallSimple extends JPanel {
   ...
   public BouncingBallSimple() {
      ...
      Thread gameThread = new Thread() {
         public void run() {
            while (true) { // Execute one update step                       //%keep looping till reach the wall
               ...
               if (ballX - ballRadius < 0) {
                  ballSpeedX = -ballSpeedX; // Reflect along normal         //%opposite direction
                  ballX = ballRadius; // Re-position the ball at the edge   //%minimum to the left wall
               } else if (ballX + ballRadius > BOX_WIDTH) {    //%rebounce right
                  ballSpeedX = -ballSpeedX;                    //%opposite direction
                  ballX = BOX_WIDTH - ballRadius;              //%maximum to the right wall
               }
